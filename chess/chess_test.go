...
func TestAvailableMoves(t *testing.T) {
	...
	t.Run("Cannot Castle While In Check — Pawn", func(t *testing.T) {
		// Black king on e8, black rook on h8 (kingside castling rights),
		// white pawn on d7 giving check diagonally to e8, white king on f1.
		// Pawn attacks on empty squares are not generated by availableMoves(),
		// so this case specifically tests that isCheck() (not a post-castle
		// origin-square check) is used to detect the in-check condition.
		// Arrange
		c, errOpts := chess.New(chess.WithFEN("4k2r/3P4/8/8/8/8/8/5K2 b k - 0 1"))
		require.Nil(t, errOpts)

		// Act
		moves := c.AvailableMoves()

		// Assert: e8g8 (kingside castle) must NOT appear.
		assert.NotContains(t, moves, "e8g8")
		// King can escape to d8, d7 (captures pawn), e7, f8, f7.
		// Rook has no legal moves (all leave king in check; pawn check
		// cannot be interposed or captured by the rook from h8).
		expectedMoves := []string{"e8d8", "e8d7", "e8e7", "e8f8", "e8f7"}
		assert.ElementsMatch(t, expectedMoves, moves)
	})
